...
1.  we can evaluate $f$ on a quantum computer by applying a unitary $U_f: |x\rangle|y\rangle \mapsto |x\rangle |y\oplus f(x)\rangle$, and
2. $f$ is a 2-to-1 function where all collisions have an interesting additional structure: there exists a bit-vector $s\in \{0,1\}^n$ such that whenever $f(x)=f(x')$ 
then $x=x' \oplus s$. 
